...
| Malformed confidence/support | Pre-merge review found that new cap checks coerced out-of-domain objects and could throw instead of returning enum issues | Compare literal bands without coercion and reject malformed confidence on a supplied, referenced antecedent before deriving its cap; preserve legal omitted candidate support (§5.5, §5.10.1–2, §7.3) |
...
- 269 contract tests pass, including 42 added in the improvement pass and six
  added during pre-merge review. Strict type checking,
...
- Independent pre-merge review checked the full PR against main and packet
  authority. Six added regressions exercise malformed confidence/support values.
  Five initially failed with coercion errors; a further regression and a
  strengthened antecedent check reproduced skipped invalid source-band validation.
  The review also checked 238
  schema/runtime combinations and found no remaining unsupported restrictions or
  accidental public-declaration changes.
- The minimal GitHub Actions workflow reproduces the documented checks on clean
  Linux and Windows checkouts, including dependency installation and tracked-file
  drift detection. It uses read-only permissions and pinned official actions.
