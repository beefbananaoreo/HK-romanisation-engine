...
// Documented materialisation corrections apply existing packet rules; see
// docs/CONTRACT-VALIDATION-AUDIT.md. No public TypeScript declaration changes.
const correctedDefinitions=['CandidateBase','CandidateEnglishForm','CandidateReading','CandidateRomanisation','CandidateTermRendering','CreationDiagnostic','MemoryChannelEnglishForm','MemoryChannelReading','MemoryChannelRomanisation','ProtectedSpan','ValueEnglishForm','ValueReading','ValueRomanisation','ValueTermRendering'];
const expected={added:['NonEmptyArray','NonEmptyReadonlyArray'],removed:[],changed:['AssembledRomanisation','Grouping','MemoryAssembledRomanisation','Versions',...correctedDefinitions].sort(),unchanged:158};
...
const output=JSON.stringify({schema,typeDelta,authority:'CHG-050 §§5.2–5.3,5.6–5.7,5.10.2; validation corrections §§5.5,5.10,5.12,7.3 and INV-17',correctedDefinitions,unrelatedExistingSchemaDefinitions:'158 unchanged',baselineSchemaSha256:createHash('sha256').update(readFileSync('baseline/schema-5.0.14.json')).digest('hex')},null,2)+'\n';
